...
export default function LoginScreen({ navigation }) {
    ...
    await firebase
      ...
      .then((resp) => {
        ...
        if (user.emailVerified) {
          ...
          setIsLoading(false);
          setSnackBarMessage(
            "Your email is not verified, please check your email and validate your email"
          );
      ...
      .catch((error) => {
        ...
        if (errorCode == "auth/weak-password") {
          ...
          if (errorCode == "auth/user-not-found") {
            setIsLoading(false);
            setSnackBarMessage2(
              "The user may have been deleted. Please register a new account"
            );
  ...
  async function checkStoreKeyChain(key) {
    ...
    if (result) {
      ...
      if (!scanned) checkDeviceForHardware();
    } else {
  ...
  return (
    <Background>
      ...
      <View style={authData.dark ? stylesDark.container : styles.container}>
        ...
        <Logo />
        <Text style={authData.dark ? stylesDark.titleLogin : styles.titleLogin}>
          Sign In
        </Text>
        <TextInput
          label="Email"
        ...
        <View
          style={
            authData.dark ? stylesDark.forgotPassword : styles.forgotPassword
          }
        >
          <TouchableOpacity
            onPress={() => navigation.navigate("ForgotPassword")}
          >
            <Text style={authData.dark ? stylesDark.label : styles.label}>
      ...
      <ButtonsLogin onPress={loginAuth}  /> */}
        <View style={authData.dark ? stylesDark.row : styles.row}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate("Register")}>
            <Text style={authData.dark ? stylesDark.link : styles.link}>
              Sign up
            </Text>
          </TouchableOpacity>
        </View>
