    ...
    const x = setInterval(() => {
      ...
      if (data.authData.timer === 'stop') {
        ...
        if (eventDate <= 0) {
          ...
          let end = moment(data.authData.timeTimer); // another date
          console.log('end', data.authData.timer);
          let duration = moment.duration(end.diff(now));
          //console.log('now',moment.utc(duration.as('milliseconds')).format('HH:mm:ss'));
          //this.setState({days, hours, mins, secs, eventDate})
          this.setState(`${moment.utc(duration.as('milliseconds')).format('HH:mm:ss')}`);
          console.log(data.authData.timeTimer);
